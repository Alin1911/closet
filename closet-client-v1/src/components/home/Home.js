...
export default function Home({closets, loading, error, recentlyViewedClosets, recommendedClosets, onTrackViewed, onToggleFavorite, authUser, onNotify, onRetry}) {
  ...
  return (
    <Container fluid className="pb-4">
      ...
      <Container className="mt-4">
        ...
        {!recommendedClosets?.length ? <p className="text-secondary">Recommendations will appear as you browse and save closets.</p> : null}
        <Row className="g-3 mb-3">
          {recommendedClosets?.map((item) => (
            <Col md={4} key={`recommended-${item.id}`}>
